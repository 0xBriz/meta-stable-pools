...
import "./pool-utils/interfaces/IRateProvider.sol";
import "./solidity-utils/helpers/BalancerErrors.sol";
import "./solidity-utils/helpers/LogCompression.sol";
...
contract MetaStablePool is StablePool, StableOracleMath, PoolPriceOracle {
        ...
        )
    {
        ...
        if (rateProvider0 != IRateProvider(address(0))) {
            (bytes32 cache, uint256 rate) = _getNewPriceRateCache(
                rateProvider0,
                params.priceRateCacheDuration[0]
            );
        ...
        if (rateProvider1 != IRateProvider(address(0))) {
            (bytes32 cache, uint256 rate) = _getNewPriceRateCache(
                rateProvider1,
                params.priceRateCacheDuration[1]
            );
    ...
    function _cachePriceRate0IfNecessary() private {
        if (_getRateProvider0() != IRateProvider(address(0))) {
            (uint256 duration, uint256 expires) = _getPriceRateCacheTimestamps(
                _getPriceRateCache0()
            );
     ...
     */
    function _getPriceRateCacheTimestamps(bytes32 cache)
        private
        pure
